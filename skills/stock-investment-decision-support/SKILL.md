...
description: 企業名だけを入力として、日本株の銘柄コードを特定し、固定の trend_viewer analysis API から直近2年の日足・テクニカル指標 JSON を取得して、中期・長期の投資判断支援レポートを作る。任天堂、トヨタ、ソニーなど企業名から銘柄分析を依頼されたときに使用する。
---

# Stock Investment Decision Support

企業名から銘柄コードを特定し、trend_viewer の analysis endpoint を使って中期・長期の投資判断支援レポートを作る。
...
保有中か未保有かは聞かない。必要なら出力側で「新規で見る場合」「保有中なら見るべき条件」を軽く分ける。
...
5. 取得に成功した企業ごとに、取得データだけで中期・長期目線の分析を行う。
...
- 中期と長期で判断が分かれる場合は、その理由を分けて書く。
- endpoint に含まれない情報を根拠にしない。必要なら「追加確認が必要」と明記する。
- 複数企業入力時の `買い転換シグナルあり` `様子見推奨` `売り転換シグナルあり` は、個別レポートの `中期判断` を要約する補助ラベルとして扱う。
- `買い転換シグナルあり` は、中期が `強気寄り` で、トレンド系とモメンタム系の改善が複数一致する場合に使う。
- `様子見推奨` は、中期が `中立` または `判断保留` の場合、または指標が割れていて方向感が弱い場合に使う。
- `売り転換シグナルあり` は、中期が `弱気寄り` で、トレンド系とモメンタム系の悪化が複数一致する場合に使う。
