...
- 1枚では厚み・背面・接合構造を判断できない対象について、正面・側面・接合部close-upから必要なReference Packを追加する。最低1枚を用意し、生成画像よりStory Beat Sheet、寸法比例表、実物資料を優先する。
...
7. 独立サブエージェントに**生成参考画像は渡さず**、全体静止画、クローズアップ、対象物の実物資料だけを渡す。一般常識と実物資料に照らし、形状、比率、向き、接地、接合の不自然さを列挙させる。
