...
- MCP可用性が`false`または`unknown`の場合は別モデルへフォールバックせず、親が担当する。
...
委任直前に動的ツール一覧で`mcp__codex__codex`の完全一致を確認する。継続対話が必要な場合だけ`mcp__codex__codex_reply`の完全一致を別途確認する。MCP可用性が`false`または`unknown`の場合は別モデルへフォールバックせず、親が担当する。
