...
- 各 holding は短期判断と分離した `long_hold_rationale`、`thesis_invalidation_or_review_trigger`、`next_review_date`、`trim_conditions`、`long_hold_governance_status` を持つ
- `long_hold_governance_status` は `complete` または `needs_user_confirmation`。未入力を短期 `thesis` から推測せず、`needs_user_confirmation` のまま記録する
- `trim_conditions` は `status=none` または `status=defined`（`trigger` `percentage` `rationale`）を必須とし、未確認時は `status=needs_user_confirmation` とする
