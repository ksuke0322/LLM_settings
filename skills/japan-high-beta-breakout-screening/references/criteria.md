...
- 決算・イベントは確認できた場合だけ、材料や注意点の補助情報として `event_advisory` に記録する
- 決算情報の未確認、欠落、取得失敗、形式不正は「イベントなし」や「安全」と解釈せず、決算情報だけで候補を除外・降格・停止しない
- 公式に確認できた正確な日付がない場合、決算直前とは推測しない。価格、出来高、相対強度、流動性などのテクニカル証拠の不足は、決算情報とは別に `technical_evidence_incomplete` として扱う
...
| リスク管理 | 10 | 損切り位置を置きやすく、公式に確認できた決算直前・希薄化・低流動性の懸念が薄い |

決算情報が取れないことだけで「材料継続性」や「リスク管理」を0点にせず、取得できた情報がある場合だけ補助的に反映する。
